...
class IndexAutoDrive(SubsystemCommand):
    ...
    def execute(self):
        ...
        If intake running:
            if Left button senses a ball and the ballcount is 0:
                generate new ball
                update ballcount (somehow)
                if ball team color:
                    if no ball in right intake:
                        left dinglebobs in 
                        right dinglebobs out
                        wait for right button response or run for amount of steps:
                            stop dinglebobs
                            set ball position
                    elif:
                        stop dinglebobs
                    ...
                    run dinglebobs until ball hits photo-electric sensor
                    set ball position
            ##vice versa for right
            if the photo sensor senses a ball and the ballcount is 0:
                set the ball count to 1
                run the index until it goes a certain amount (need to measure)
                set the index speed to zero
            elif the photo sensor senses a ball and the ballcount is 1:
