...
class Shooter_Targeting:
    ...
    def calculate_velocity(velocity_up, distance_to_hub):
        time1 = Shooter_Targeting.time_up(velocity_up)
        dist_up = Shooter_Targeting.distance_up(time1, velocity_up)
        dist_down = dist_up - height_difference
        time2 = Shooter_Targeting.time_down(dist_down)
        total_time = time1 + time2
        velocity_sideways = Shooter_Targeting.velocity_horizontal(distance_to_hub, total_time)
    ...
    def gradient_velocity(distance_to_hub, velocity_up=10.0, step_size=0.1):
        try:
            current_energy = Shooter_Targeting.calculate_energy(Shooter_Targeting.calculate_velocity(velocity_up, distance_to_hub))
            increase_energy = Shooter_Targeting.calculate_energy(Shooter_Targeting.calculate_velocity(velocity_up + step_size, distance_to_hub))
            decrease_energy = Shooter_Targeting.calculate_energy(Shooter_Targeting.calculate_velocity(velocity_up - step_size, distance_to_hub))

            if increase_energy < current_energy:
                return Shooter_Targeting.gradient_velocity(distance_to_hub, velocity_up + step_size, step_size)

            if decrease_energy < current_energy:
                return Shooter_Targeting.gradient_velocity(distance_to_hub, velocity_up - step_size, step_size)

            return Shooter_Targeting.calculate_velocity(velocity_up, distance_to_hub)
        except:
            return Shooter_Targeting.gradient_velocity(distance_to_hub, velocity_up + step_size, step_size)
